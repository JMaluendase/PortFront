  ...
  <div class="colorpanel">
    ...
    <div class="container">
      <div class="formgrid grid">
        ...
        <!-- DataTable -->
        <div class="field col-12 md:col-12">
          <DataTable v-show="!cargaskeletons" :value="extractos" stripedRows showGridlines tableStyle="min-width: 50rem"
            responsiveLayout="scroll" class="small-datatable">
            <ColumnBasic field="NumVehiculo" header="Vehículo"></ColumnBasic>

            <ColumnBasic field="Periodo" header="Periodo"></ColumnBasic>

            <ColumnBasic header="Ingresos">
              <template #body="slotProps">
                <BadgePrime severity="info">{{
                  UTILS.formatCurrency(slotProps.data.TIngresos)
                }}</BadgePrime>
              </template>
            </ColumnBasic>

            <ColumnBasic header="Egresos">
              <template #body="slotProps">
                <BadgePrime severity="danger">{{
                  UTILS.formatCurrency(slotProps.data.TEgresos)
                }}</BadgePrime>
              </template>
            </ColumnBasic>

            <ColumnBasic header="Saldo">
              <template #body="slotProps">
                <BadgePrime v-if="slotProps.data.TIngresos - slotProps.data.TEgresos > 0" severity="success">{{
                  UTILS.formatCurrency(
                    slotProps.data.TIngresos - slotProps.data.TEgresos
                  )
                }}</BadgePrime>
                <BadgePrime v-else severity="danger">{{
                  UTILS.formatCurrency(
                    slotProps.data.TIngresos - slotProps.data.TEgresos
                  )
                }}</BadgePrime>
            ...
            <ColumnBasic header="Planilla">
              <template #body="slotProps">
                <ButtonPrime icon="pi pi-download" aria-label="Cancel" label="Descargar" rounded outlined
                  @click="verPlanilla(slotProps.data.Planilla)" />
              </template>
            </ColumnBasic>

            <ColumnBasic header="Extractos">
              <template #body="slotProps">
                <ButtonPrime icon="pi pi-download" severity="danger" aria-label="Cancel" label="Descargar" rounded
                  outlined @click="verExtracto(slotProps.data.Extracto)" />
              </template>
            </ColumnBasic>
          </DataTable>
        </div>
...
export default {
  ...
  methods: {
    ...
    verExtracto: function (nombre) {
      var link = this.axios.defaults.baseURL + "descargar/" + nombre + "/";
      //var link = "http://127.0.0.1:8000/api/pdf_extractos/" + nombre;
